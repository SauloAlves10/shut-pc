;
; AutoHotkey Version: 1.2 portable
; Language:       Portuguese
...
IniRead, Cond, %A_WorkingDir%\shutpc.cfg, Condição, cond
If Cond = 0
{
	Goto, ShutPC
}
...
Vazio =
IfNotExist, %A_WorkingDir%\shutpc.cfg
{
	IniWrite, 1, %A_WorkingDir%\shutpc.cfg, Opções de Ação, opcoes
	IniWrite, %Vazio% ,%A_WorkingDir%\shutpc.cfg, Mensagem, mens	
}
...
; Verifica se há senha para iniciar o aplicativo
	...
	IniWrite, 0, %A_WorkingDir%\shutpc.cfg, Condição, cond	
...
IniRead, ShutOpt, %A_WorkingDir%\shutpc.cfg, Opções de Ação, opcoes
Gui, Add, DropDownList, AltSubmit Choose%ShutOpt% vShutOpt, Nenhum|Desligar|Reiniciar|Logoff|Hibernar
	
IniRead, TimeIni, %A_WorkingDir%\shutpc.cfg, Horário da Ação, timeini
...
IniRead, AtivData, %A_WorkingDir%\shutpc.cfg, Ativar Data, ativdata
If AtivData = 1
{
	Ticado3 = Checked
}
...
IniRead, Data, %A_WorkingDir%\shutpc.cfg, Data, data
Gui, Add, DateTime, %Ticado4% w230 vMyDateTime Choose%Data%, LongDate

IniRead, AtivMens, %A_WorkingDir%\shutpc.cfg, Ativar Mensagem, ativmens
If AtivMens = 1
{
	Ticado = Checked
}
...
IniRead, MensChoose, %A_WorkingDir%\shutpc.cfg, Mensagem, mens
Gui, Add, Edit, %Ticado2% w230 r2 Limit100 vMyEdit, %MensChoose%
	
IniRead, MensTime, %A_WorkingDir%\shutpc.cfg, Horário da Mensagem, menstime
...
IniWrite, %ShutOpt%, %A_WorkingDir%\shutpc.cfg, Opções de Ação, opcoes
	
IniWrite, %Data%, %A_WorkingDir%\shutpc.cfg, Ativar Data, ativdata

IniWrite, 1, %A_AppData%\ShutPC\shutpc.cfg,Anti Shut Exec, antishutexec
	
MyDateTime := SubStr(MyDateTime, 1, 8)
IniWrite, %MyDateTime%, %A_WorkingDir%\shutpc.cfg, Data, data
...
IniWrite, %TimeIni%, %A_WorkingDir%\shutpc.cfg, Horário da Ação, timeini

IniWrite, %Mensagem%, %A_WorkingDir%\shutpc.cfg, Ativar Mensagem, ativmens

IniWrite, %MyEdit%, %A_WorkingDir%\shutpc.cfg, Mensagem, mens
...
IniWrite, %MensTime%, %A_WorkingDir%\shutpc.cfg, Horário da Mensagem, menstime
...
IniRead, IniWin, %A_WorkingDir%\shutpc.cfg, Iniciar com o Windows, iniwin
If IniWin = 1
{
	Ticado5 = Checked
}
...
Gui, 2:Add, Checkbox, %IniWindows% y28 x17  %Ticado5% vIniciar, Iniciar com o Windows (disponível na versão com instalador)
	
IniRead, IniWin, %A_WorkingDir%\shutpc.cfg, Aviso Sonoro, sonoro
If IniWin = 1
{
	Ticado6 = Checked
}
...
Gui, 2:Show,, Opções - Shut PC! 1.2 Portable
return
...
Gui, 2:Add, Text, y25 x20, Shut PC! 1.2 Portable`n `nEste é um software gratuito, pode ser`ndistribuído e utilizado livremente.`nNenhuma taxa pode ser cobrada por ele.`n`nBom proveito,`n`nSaulo Alves.
Gui, 2:Add, Button, x100 w60 Default, Shut It!
Gui, 2:Show,, Sobre - Shut PC! 1.2 Portable
return
...
Gui, 2: Submit, NoHide
	...
	If Iniciar = 1
	{
		...
		IniWrite, 1, %A_WorkingDir%\shutpc.cfg, Condição, cond
		IniWrite, 1, %A_WorkingDir%\shutpc.cfg, Iniciar com o Windows, iniwin
	}
	Else
	{
		RegDelete, HKEY_LOCAL_MACHINE, SOFTWARE\Microsoft\Windows\CurrentVersion\Run, ShutPC
		IniWrite, 0, %A_WorkingDir%\shutpc.cfg, Condição, cond
		IniWrite, 0, %A_WorkingDir%\shutpc.cfg, Iniciar com o Windows, iniwin
	}
		
	If WarnSound = 1
	{
		IniWrite, 1, %A_WorkingDir%\shutpc.cfg, Aviso Sonoro, sonoro
	}
	Else
	{
		IniWrite, 0, %A_WorkingDir%\shutpc.cfg, Aviso Sonoro, sonoro
	}
...
ShutPC:
Menu, tray, Tip, Shut PC! 1.2 Portable
Menu, tray, NoStandard
...
IniRead, TimeIni, %A_WorkingDir%\shutpc.cfg, Horário da Ação, timeini
IniRead, ShutOpt, %A_WorkingDir%\shutpc.cfg, Opções de Ação, opcoes
IniRead, ContMens, %A_WorkingDir%\shutpc.cfg, Ativar Mensagem, ativmens
IniRead, Mens, %A_WorkingDir%\shutpc.cfg, Mensagem, mens
...
IniRead, ContData, %A_WorkingDir%\shutpc.cfg, Ativar Data, ativdata
If ContData = 1
{
	IniRead, DataShut, %A_WorkingDir%\shutpc.cfg, Data, data
	...
	If ContMens = 1
	{
		...
		If AtivData = 1
		{
			...
			{	
				...
				{
					IniRead, Avsonoro, %A_WorkingDir%\shutpc.cfg, Aviso Sonoro, sonoro
